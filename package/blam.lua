...
function triangle_intersects_point(point, v0, v1, v2)
...
function get_lowest_root(root, a, b, c, max)
...
function check_triangle(packet, p1, p2, p3, id)
    local pn = vec3_normalize(vec3_cross(vec3_sub(p2, p1), vec3_sub(p3, p1)))
    if vec3_dot(pn, packet.e_norm_velocity) > 0 then
    end
    local t0 = 0
    local embedded_in_plane = false
    local signed_dist_to_plane = vec3_dot(packet.e_base_point, pn) - vec3_dot(pn, p1)
    local normal_dot_vel = vec3_dot(pn, packet.e_velocity)
    ...
    if not embedded_in_plane then
        local plane_intersect = vec3_sub(packet.e_base_point, pn)
...
function check_collision(packet, tris, ids)
...
function blam.response_get_tris(position, velocity, radius, query)
...
local function collide_with_world(packet, e_position, e_velocity, tris, ids)
    ...
    do
        ...
        while i < 3 do
            ...
            local p = { position = packet.intersect_point, normal = pn }
            ...
            elseif i == 1 and first_plane then
...
local function sub_update(packet, position, tris, ids)
    local e_position = vec3_clone(packet.e_position)
    local e_velocity = vec3_clone(packet.e_velocity)

    local final_position = collide_with_world(packet, e_position, e_velocity, tris, ids)
...
function blam.response_check(tri_cache, id_cache, position, velocity, radius, query)
    ...
    if query then
        tri_cache = blam.response_get_tris(min, max, packet.r3_velocity, query)
        id_cache = nil
    ...
    if packet.found_collision then
        local touch_point = vec3_add(packet.e_position, vec3_scale(packet.e_velocity, packet.intersect_time))
...
function blam.response_update(position, velocity, radius, query, substeps)
    ...
    local tri_cache = blam.response_get_tris(position, velocity, radius, query)
    ...
    for i=1, substeps do
        ...
        sub_update(packet, packet.r3_position, tri_cache)
